...
# Maximum radius for distributing particles in R_Delta units
RMAX = 5.0
...
_inv_u = np.array(
    [
        [
            optimize.fsolve(
                lambda r: normalizedNFWMass(c, r)
                * normalizedNFWMass(c, 1.0)
                / normalizedNFWMass(c, RMAX)
                - u,
                u,
            )[0]
            for c in _c
