...
def model1(L, z):
    ...
    if z < zbreak:
        ...
        phi_starz = (
            phi_star0_1 * (1.0 + z) ** (-epsilon) * (1.0 + zbreak) ** (2 * epsilon)
        )
...
def model1Fast(L, z):
    ...
    if z < zbreak:
        ...
        phi_starz = (
            phi_star0_1_u * (1.0 + z) ** (-epsilon) * (1.0 + zbreak) ** (2 * epsilon)
        )
...
def model3(L, z):
    ...
    L_starz = (
        ...
        ** (
            log10_L_starIn
            + ((1.5 / (1.0 + z)) ** beta) * (log10_L_star05 - log10_L_starIn)
        )
        * u.erg
        / u.s
    )
    ...
    return (
        ...
        * (L / L_starz) ** alpha_3
        * 1.0
        / (1.0 + (np.exp(1.0) - 1.0) * (L / L_starz) ** Delta)
    )
...
def model3Fast(L, z):
    ...
    L_starz = 10 ** (
        log10_L_starIn + ((1.5 / (1.0 + z)) ** beta) * (log10_L_star05 - log10_L_starIn)
    )
